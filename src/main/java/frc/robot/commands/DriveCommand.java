...
public class DriveCommand extends CommandBase {
    ...
    double curSpeedT = 0;
    final double speedInc = 1.0; //how long it takes the robot to reach full speed (in seconds)
    ...
    private double magicMotion() {
        ...
        if(curSpeedT != 0) {
            ...
            if(Math.abs(currentZero - Robot.gyro.getYaw()) > magicMotionDeadZoneThresh) {
                //If the difference beteen the last set currentZero direction and the current direction
    ...
    private double moveCalculation(final double controller, final double multiplication, final double currentValue) {
        ...
        if (controller < stickDeadZoneThresh && controller > -stickDeadZoneThresh) {
            finalSpeed = 0;
        }
